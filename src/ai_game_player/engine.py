...
class GamePlayerEngine:
    ...
    def step(self,observation:ScreenObservation,candidates:list[ActionCandidate])->ActionDecision:
        decision=self.provider.choose(self.evaluator.evaluate(observation,candidates)); self.history.append(observation,decision); return decision
