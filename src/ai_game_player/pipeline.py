...
class DecisionPipeline:
    def __init__(self, source: ObservationSource, game_directory: Path, provider=None) -> None:
        self.source=source; self.ocr=OcrTextCandidateDetector(); self.merger=CandidateMerger(); self.engine=GamePlayerEngine(game_directory,provider); self.executor=ActionExecutor()
    ...
    def run_and_execute(self, ocr_texts=None, purpose: str="", personality: str="")->ExecutionResult:
        ...
        return self.executor.execute(selected)