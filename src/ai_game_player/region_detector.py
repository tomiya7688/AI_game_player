...
class ConfiguredRegionDetector:
    ...
    def detect(self, observation: ScreenObservation) -> list[ActionCandidate]:
        ...
        if not isinstance(regions, list):
            raise ValueError("automation.json regions must be an array")
        result = []
        for region in regions:
            ...
            if width <= 0 or height <= 0:
                continue
            result.append(ActionCandidate(str(region["action_id"]), str(region.get("kind", "click")), str(region.get("label", region["action_id"])), x + width // 2, y + height // 2, float(region.get("confidence", 1.0)), bool(region.get("dangerous", False)), (x, y, width, height)))
        return result