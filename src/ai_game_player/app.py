...
class Application:
    def __init__(self, root: tk.Tk) -> None:
        ...
        self.obs=tk.Text(frame,height=10); self.obs.pack(fill=tk.BOTH,expand=True)
        ...
        ttk.Label(frame,text="操作候補JSON（安全評価を通過した候補だけ判断）").pack(anchor=tk.W,pady=(8,0))
        self.actions=tk.Text(frame,height=10); self.actions.pack(fill=tk.BOTH,expand=True)
        ...
        self.result=ttk.Label(frame,text="待機中"); self.result.pack(anchor=tk.W)
    def run(self) -> None:
        try:
            observation=ScreenObservation(**json.loads(self.obs.get("1.0",tk.END)))
            candidates=[ActionCandidate(**item) for item in json.loads(self.actions.get("1.0",tk.END))]
            decision=GamePlayerEngine(Path("data/games/sandbox")).step(observation,candidates)
