...
from ai_game_player.pipeline import DecisionPipeline`nfrom ai_game_player.provider import OllamaProvider, RuleProvider
...
class Application:
    def __init__(self,root:tk.Tk)->None:
        ...
        frame=ttk.Frame(root,padding=10); frame.pack(fill=tk.BOTH,expand=True)`n        self.provider= tk.StringVar(value="ローカル規則"); self.model=tk.StringVar(value="gemma3:4b"); self.endpoint=tk.StringVar(value="http://127.0.0.1:11434")`n        settings=ttk.Frame(frame); settings.pack(fill=tk.X); ttk.Label(settings,text="Provider").pack(side=tk.LEFT); ttk.Combobox(settings,textvariable=self.provider,values=("ローカル規則","Ollama"),state="readonly",width=12).pack(side=tk.LEFT,padx=5); ttk.Label(settings,text="モデル").pack(side=tk.LEFT); ttk.Entry(settings,textvariable=self.model,width=16).pack(side=tk.LEFT,padx=5); ttk.Label(settings,text="Endpoint").pack(side=tk.LEFT); ttk.Entry(settings,textvariable=self.endpoint,width=28).pack(side=tk.LEFT,padx=5)
    ...
    def run(self)->None:
        try:
            ...
            provider=OllamaProvider(self.model.get(),self.endpoint.get()) if self.provider.get()=="Ollama" else RuleProvider()`n            pipeline=DecisionPipeline(MemorySource(observation,candidates),Path("data/games/sandbox"),provider); decision=pipeline.run(purpose="画面の役割を理解する",personality="好奇心旺盛")
